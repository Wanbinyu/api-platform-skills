...
description: >
  ...
  mass assignment). Use for public/authenticated APIs, "/api-secure", "BOLA", "BFLA",
  "IDOR", "API security review". Defensive findings only — not red-team exploits or
  binary audit workflows (those belong to specialist security skill packs).
---
...
2. **Per authenticated resource op**  
   - Object-level (BOLA/IDOR): user A + user B’s id?  
